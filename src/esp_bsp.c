...
static esp_err_t bsp_display_brightness_init(void)
{
    ...
    /*
     * 13 bits en vez de 10, y 2 kHz en vez de 5 kHz.
     *
     * Con 10 bits, un 5 % de brillo percibido cae en un duty de apenas 1 o 2
     * pasos: no queda resolucion donde mas falta, que es la zona baja para uso
     * nocturno. Con 13 bits hay 8192 pasos. La frecuencia se baja a 2 kHz para
     * que a duty muy pequeno el pulso siga siendo lo bastante largo como para
     * que el driver del LED lo reproduzca (sigue muy por encima del parpadeo
     * visible). A 13 bits el maximo posible serian 9,7 kHz, asi que 2 kHz cabe.
     */
...
esp_err_t bsp_display_brightness_set(int brightness_percent)
{
    ...
    /*
     * Correccion gamma. El duty del PWM es proporcional a la LUZ emitida, pero
     * el ojo no responde de forma lineal: un 10 % de luz se percibe como un
     * ~35 % de brillo. Con el mapeo lineal original, bajar el deslizador al
     * 10 % apenas oscurecia la pantalla y casi todo el recorrido util se
     * amontonaba en la zona alta.
     *
     * Elevando a 2,2 (el exponente estandar de percepcion) el deslizador pasa a
     * ser aproximadamente lineal en brillo PERCIBIDO, y la mitad inferior sirve
     * de verdad para uso nocturno:
     *
     *      100 % -> 8191      20 % ->  237      5 % ->  11
     *       50 % -> 1777      10 % ->   52      2 % ->   2
     */
    const uint32_t max_duty = (1u << 13) - 1u;
    uint32_t duty_cycle = 0;
    if (brightness_percent > 0) {
        float lin = powf((float)brightness_percent / 100.0f, 2.2f);
        duty_cycle = (uint32_t)(max_duty * lin + 0.5f);
        if (duty_cycle == 0) duty_cycle = 1;   /* que un 1 % no sea apagar del todo */
    }

    ESP_LOGI(TAG, "Setting LCD backlight: %d%% (duty %lu/%lu)",
             brightness_percent, (unsigned long)duty_cycle, (unsigned long)max_duty);
...
esp_err_t bsp_display_set_flipped(bool flipped)
{
    /* Girar la imagen 180 grados es alternar entre las dos rotaciones
     * apaisadas. Imagen y tactil TIENEN que cambiar juntos: si solo girase la
     * imagen, tocar arriba respondaria abajo y los ajustes quedarian
     * inservibles justo despues de activar la opcion. */
    lv_display_rotation_t rot = flipped ? LV_DISPLAY_ROTATION_270 : LV_DISPLAY_ROTATION_90;

    lvgl_port_set_sw_rotation(disp, rot);

    if (tp != NULL && tp->config.user_data != NULL) {
        ((bsp_touch_int_t *)tp->config.user_data)->rotate = rot;
    }
